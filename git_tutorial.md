...
Чтобы сдёлать копию чьего-то другого удаленного репозитория в свой удаленный репозиторий на GitHub, нужно на GitHub зайти в этот репозиторий (например ***https://github.com/cheklinim/seminar3_HW***) и нажать на кнопку "Fork". 

Далее, чтобы новый удаленный репозиторий клонировать на своё локальное хранилище, нужно выполнить команду:

        git clone <ссылка на удалённый репозиторий>

---

## Предложение изменений в чужой удаленный репозиторий

Если мы хотим предложить изменения для чужого удаленного репозитория, то нужно сделать:

* 1. В своем локальном репозитории(копии чужого) сделать новую ветку и перейти на нее.
* 2. В новой ветке внести необходимые изменения и сделать ***commit***
* 3. Запушить изменения в удаленный репозиторий ***git push***

*Примечание: на шаге 3 после выполнения push произойдет ошибка, так как на удаленном репозитории нет ветки, в которую мы пытаемся сделать push. Git сам выдаст подсказку, какую команду следует использовать, например: ***git push --set-upstream origin remoted***.*

Далее в на Github на странице клонированного ранее удаленного репозитория появится кнопка "Compare & Pull request".
Нужно ее нажать, написать комментарий и предложить свои измения автору удаленного репозитория.
