...
### Клонирование репозиторя. Вид Fork.

Чтобы скопировать чужой репозиторий к себе в аккаунт и работать с ним, сужествуют некоторые правила.
1. Ищем интересующий нас репо на сайте github.com.
2. Заходим на него. Нажимаем иконку Fork => Create fork. В итоге у нас в аккаунте клон чужого репо под видом форка. Все изменения, производимые нами в этом репо, в оригинале не будут отображаться.
3. Заходим на наш клонированый репо. Нажимаем кнопку Code, копируем ссылку.
4. Заходим в VSC. Там сверху кликаем по меню File => Open folder... => создаём новую папку и даём ей название => открываем в этой папке новый терминал => в терминале пишем команду 

    *git clone <скопированная ссылка клонированного репо>*

Происходит клонирование папки репо.
5. Нажимаем на клонированную папку правой кнопкой мыши и выбираем строку Open in Integrate Terminal.
6. В терминале создаём новую ветку и уже в ней работаем: вносим изменения, сохраняем, комитим.
7. После окончания нужных нам изменений нужно запушить наш репо на сайт. Для этого вводи команду git push. При работе в комманде перед этой командой лучше сделать git pull, чтобы не было конфликтов при пушении. Гит предлагает нам ввести строчку для пушения из допветки. Копируем эту строку и вводим

    *git push --set-upstream <name_branch>*

Этим мы сообщаем, что пушим не в master  ветку, а в свою созданную.
8. Заходим на сайт.Обновляем. Нажимаем зеленую кнопку "Compare & pull request". Это есть кнопка сравнения и запроса на ветвление.
9. В открывшемся окне даём название нашему пулу. Пишем заголовок сообщения и само сообщение.
10. Затем нажимаем кнопку "Create pull request"
Наши изменения отправлены хозяину репозитория.
