...
### Скачивание всего из репозитория на github в локальный репозиторий

Для того чтобы скачать все из репозитория на github в наш локальный репозиторий используется команда:

    git pull

**ВАЖНО:** автоматически делается merge с нашей версией. Поэтому могут возникнуть конфликты, их нужно будет разрешить как описывалось в уроке 2.

**Общая рекомендация:** Читаем сообщения в терминале от git, он подсказывает всё что делаем не так
